...
export function SegmentedControl({ items = [], value, onChange, size = 'sm', fullWidth = true, style, ariaLabel }) {
  const h = size === 'md' ? 'var(--control-h)' : 'var(--control-h-sm)';
  return (
    <div role="group" aria-label={ariaLabel} style={{ display: fullWidth ? 'grid' : 'inline-grid', gridAutoFlow: 'column', gridAutoColumns: 'minmax(0, 1fr)', gap: '1px', background: 'var(--line-hairline)', boxShadow: 'inset 0 0 0 1px var(--line-hairline)', ...style }}>
      {items.map(it => {
        const on = it.id === value;
        return (
          <button
            key={it.id} type="button" aria-pressed={on} onClick={() => onChange && onChange(it.id)} title={it.title}
            style={{
