...
const terrainColors = {
  red: ['rgba(239,68,68,.28)', '#ef4444'], amber: ['rgba(245,158,11,.26)', '#f59e0b'],
  blue: ['rgba(59,130,246,.25)', '#60a5fa'], purple: ['rgba(168,85,247,.25)', '#c084fc'],
  emerald: ['rgba(16,185,129,.25)', '#34d399'], green: ['rgba(34,197,94,.25)', '#4ade80']
};
...
function CharacterResources({ character }) {
  return <div className="presenter-resource-grid">
    {(character.resources || []).map(resource => <div className={`presenter-resource ${resource.value <= 0 ? 'empty' : ''}`} key={resource.name}>
      <span>{resource.name === '动作' ? '⚔️' : resource.name === '附赠动作' ? '⚡' : /法术|魔法/.test(resource.name) ? '🔮' : '◆'} {resource.name}</span>
...
function CharacterPanel({ character }) {
  ...
  return <aside className="presenter-character-panel">
    ...
    <div className="presenter-vitals"><span>❤️ <b>{character.hp}</b>/{character.maxHp}</span><span>🛡️ AC <b>{character.ac}</b></span><span>🏃 <b>{Math.round(character.combatSpeedRemaining)}</b>/{character.speed}ft</span></div>
    <div className="presenter-hp"><span style={{ width: `${hpPercent}%` }} /></div>
...
function MapStage({ snapshot, fill = false }) {
  ...
  return <div className={`presenter-map ${fill ? 'fill' : ''}`} ref={stageRef} onWheel={wheel} onPointerDown={pointerDown} onPointerMove={pointerMove} onPointerUp={pointerUp} onPointerCancel={pointerUp}>
    <div className="presenter-map-title">🗺️ {map.name}<small>{settings.cameraMode === 'follow-active' ? '镜头跟随当前角色' : settings.cameraMode === 'follow-dm' ? '跟随主持人镜头' : '独立镜头 · 滚轮缩放'}</small></div>
    <div className="presenter-map-world" style={{ width: map.width * grid, height: map.height * grid, transform: `translate(${view.x}px,${view.y}px) scale(${view.scale})`, backgroundImage: `${map.bgUrl ? `linear-gradient(rgba(5,7,12,.2),rgba(5,7,12,.2)),url("${map.bgUrl.replaceAll('"', '%22')}")` : ''},linear-gradient(rgba(148,163,184,.1) 1px,transparent 1px),linear-gradient(90deg,rgba(148,163,184,.1) 1px,transparent 1px)`, backgroundSize: `${map.bgUrl ? 'cover,' : ''}${grid}px ${grid}px,${grid}px ${grid}px` }}>
      ...
      {map.terrainAreas.map(area => { const colors = terrainColors[area.color] || terrainColors.purple; const circle = area.type === 'circle'; return <div className="presenter-terrain" key={area.id} title={area.name} style={{ left: (area.gridX - (circle ? area.radius : 0)) * grid, top: (area.gridY - (circle ? area.radius : 0)) * grid, width: (circle ? area.radius * 2 : area.width) * grid, height: (circle ? area.radius * 2 : area.height) * grid, borderRadius: circle ? '50%' : 8, background: colors[0], borderColor: colors[1] }}><span>{area.name}</span></div>; })}
...
function PartyScene({ campaign }) {
  return <div className="presenter-party"><header><small>ADVENTURING PARTY</small><h1>队伍状态</h1></header><div className="presenter-party-grid">{campaign.characters.filter(character => character.type === 'PC').map(character => <article key={character.id}><div><span className="presenter-avatar large pc">{character.name.slice(0, 2)}</span><h2>{character.name}</h2></div><div className="presenter-vitals"><span>❤️ {character.hp}/{character.maxHp}</span><span>🛡️ {character.ac}</span><span>🏃 {character.speed}ft</span></div><div className="presenter-hp"><span style={{ width: `${Math.max(0, Math.min(100, character.hp / character.maxHp * 100))}%` }} /></div><div className="presenter-conditions">{character.conditions.map(condition => <span key={condition.id}>{condition.name}</span>)}</div><CharacterResources character={character} /></article>)}</div></div>;
}
...
function PresenterContent({ snapshot }) {
  ...
  const activeId = campaign.combatTurnOrder[campaign.currentTurnIndex]?.id;
  const active = campaign.characters.find(character => character.id === activeId);
  if (settings.scene === 'pause') return <div className="presenter-pause"><div>⏸</div><h1>{settings.pausedMessage}</h1><p>DMForge · 战役直播展示</p></div>;
  if (settings.scene === 'story') return <div className="presenter-story" style={campaign.map?.bgUrl ? { backgroundImage: `linear-gradient(rgba(4,5,10,.35),rgba(4,5,10,.88)),url("${campaign.map.bgUrl.replaceAll('"', '%22')}")` } : {}}><div><small>STORY SCENE</small><h1>{settings.storyTitle}</h1><p>{settings.storySubtitle}</p></div></div>;
...
export default function PresenterPage() {
  ...
  if (!snapshot) return <main className="presenter-root presenter-waiting"><div className="presenter-logo">◇</div><h1>DMForge 直播展示</h1><p>{sessionId ? '正在等待主持人控制端…' : '缺少展示会话，请从 DMForge 设置中打开。'}</p></main>;
  return <main className={`presenter-root ${snapshot.settings.hideCursor ? 'hide-cursor' : ''}`} style={{ '--presenter-scale': snapshot.settings.fontScale }}>
    ...
    <button className="presenter-fullscreen" title="切换全屏" onClick={() => document.fullscreenElement ? document.exitFullscreen?.() : document.documentElement.requestFullscreen?.()}>⛶</button>
  </main>;
}
